def req(strs)-> str: # функция переставляет знаки по одному по очереди и считает подходит данная расстановка знаков для достижения цели или нет, если да, то печатает строку и выхожит из рекурсия, если нет, вызывает сама себя дальше
    count: int
    count = strs.count(' ')
    if count > 0:
        req(strs.replace(' ', '+', 1))
